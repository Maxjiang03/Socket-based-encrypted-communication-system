...
while True:
    ...
    print('4.获得会话密钥')
    print('5.利用会话密钥进行多人通信')
    print('6.利用会话密钥和服务端通信')
    print('7.退出')
    print('--------------------------------------------------------')
    ...
    if toSend == '5':
        ...
        print('输入exit退出与服务器的对话')
        while True:
            chat = input('说点啥')

            en_chat = DesEncrypt(chat, wk)
            dataSocket.send(en_chat)
            if chat == 'exit':
                break
            re_chat = dataSocket.recv(BUFLEN)
            ren_chat = DesDecrypt(re_chat, wk)
            print('对方说')
            print(ren_chat)
    elif toSend == '7':
        break
